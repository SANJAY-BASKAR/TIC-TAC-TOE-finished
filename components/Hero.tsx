...
export default function Hero() {
  ...
  return (
      <>
        <Header />
        <main className={`flex flex-col items-center justify-center min-h-screen bg-custom-blue z-20 ${winner === "O" ? "bg-green-500" : winner === "X" ? "bg-red-500" : ""} transition-colors duration-500 ease-in-out`}>
          {!winner && (
              <div className="flex flex-col md:flex-row items-center justify-between w-full max-w-4xl p-4 lg:bg-purple-950 shadow-lg rounded-lg z-10">
                <div className={`flex flex-col items-center justify-center w-24 h-18 md:w-1/4 md:h-auto p-4 m-2 rounded-md ${turnO ? "bg-green-500" : "bg-white"}`}>
                ...
                <div className={`flex flex-col items-center justify-center w-24 h-18 md:w-1/4 md:h-auto p-4 m-2 rounded-md ${!turnO ? "bg-red-500" : "bg-white"}`}>
                  <span className="text-white font-bold">Computer</span>
                </div>
              </div>
          )}
          {winner && <div className="mt-4 text-lg lg:text-[5rem] font-bold text-black">Congratulations! The winner is {winner}</div>}
          {draw && <div className="mt-4 text-lg font-bold text-yellow-500">Game was a Draw.. Shift turns</div>}
          <div className="mt-12">
